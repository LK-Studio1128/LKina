...
public:
	...
	void compute_ad4_maps(const std::string& receptor_pdbqt_path,
					  double center_x, double center_y, double center_z,
					  double size_x, double size_y, double size_z,
					  const std::string& autogrid4_exe="autogrid4",
					  double granularity=0.375,
					  const std::string& map_output_dir="");
	std::string get_last_ad4_map_prefix() const { return m_last_ad4_map_prefix; }
	...
	void set_reactive_options(const ReactiveOptions& opts);
	void clear_reactive_options();
	void set_zn_mode(bool enabled) { m_zn_mode = enabled; }
	void set_metal_mode(ag4_metal_mode mode) { m_metal_mode = mode; }
	void add_metal_mode(ag4_metal_mode mode) { m_extra_metal_modes.push_back(mode); }
	void set_metal_soft_weight(float w) { m_metal_soft_weight = w; }
	void set_ligand_metal_geometry_weight(float w) { m_ligand_metal_geometry_weight = w; }
	void clear_extra_metal_modes() { m_extra_metal_modes.clear(); }
	void set_ad4_receptor_path(const std::string& path) { m_ad4_receptor_path = path; }

private:
	// AutoDock4Zn metal coordination mode
	bool             m_zn_mode                 = false;
	ag4_metal_mode   m_metal_mode              = ag4_metal_mode::none;
	float            m_metal_soft_weight       = 0.0f;
	float            m_ligand_metal_geometry_weight = 0.0f;
	std::vector<ag4_metal_mode> m_extra_metal_modes;  // multi-metal site support
	// E1: cached metal atom positions from receptor for coordination geometry REMARK
	struct metal_position { std::string type; double x, y, z; };
	std::vector<metal_position> m_metal_cache;
	bool             m_metal_cache_loaded      = false;
	void             load_metal_cache_if_needed();
	// Receptor PDBQT path stored for reactive anchor resolution when using --maps
	std::string      m_ad4_receptor_path;
	// reactive covalent docking
	ReactiveOptions  m_reactive_options;
	bool             m_reactive_enabled        = false;
	reactive_payload m_reactive_payload;
	bool             m_reactive_payload_ready  = false;
	sz resolve_ligand_reactive_atom(const std::string& spec) const;
	void finalize_reactive_state_if_possible();
	// model and poses
	model m_receptor;
	model m_model;
	...
	std::string m_last_ad4_map_prefix;
	// global search
	int m_cpu;
	int m_seed;
	// others
	...
	fl ligand_metal_geometry_penalty(const model& m, std::string* detail = 0) const;
