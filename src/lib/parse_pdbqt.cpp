...
parsed_atom parse_pdbqt_atom_string(const std::string& str) {
    ...
    // "W" in PDBQT means Tungsten (Wolfram element); AD_TYPE_W=30 is the
    // internal hydrated-water probe added programmatically, not via type string.
    if (name == "W" && ad == AD_TYPE_W) ad = AD_TYPE_Tg;
