...
**结果：** 我们提出 **LKina**，一个在引擎层面为 AutoDock Vina 1.2.7 增加金属配位对接与共价响应式对接能力的开源 C++ 扩展。LKina 将 AD4 原子类型表从约 22 种扩展至 **113 种**（覆盖 80 余种金属/类金属），内嵌 AutoGrid 4.2 格点生成器的 C++ 重实现（单次运行输出 **108** 张格点，去除外部 `autogrid4` 依赖），引入四类配位伪原子（**TZ/SQ/MH/JT**）与启发式键价和（BVS）氧化态推断，并实现带显式 NAC 判别、六种反应预设与两阶段搜索选项的四层共价响应框架（**P1–P4**）。本文证据分三个层级报告。*软件与数值验证*：17/17 项回归检查全部通过，解析式反应梯度与有限差分的最大偏差 ≤ 4.3 × 10⁻⁸，标准对接逐位复现 Vina 1.2.7 的 mode-1 能量（1HVR −14.54；3PTB −6.202 kcal/mol）。*合成体系功能覆盖*：在 110 模式金属配位基准上 LKina 完成 **110/110** 次对接（Vina 1.2.7 仅能解析 34/110，其余在原子类型解析阶段失败），配位距离与理想值之差在 0.5 Å 以内者 **108/110**（平均 |d−d₀| = 0.20 Å）；六种共价预设全部端到端跑通，且 NAC 判别具备真实几何分辨力。*真实体系对比*：在 104 例 PDB 金属蛋白重对接数据集（Zn²⁺ 22 / Fe³⁺ 47 / Cu²⁺ 35）上，启用配位模型后 87/96（**90.6%**）的体系最佳姿态将配体供体原子置于金属 3.0 Å 配位球内，而同一引擎关闭金属项为 27/97（27.8%）、Vina 1.2.7 为 50/96（52.1%）；三种引擎的 top-1 RMSD ≤ 2.0 Å 成功率在精简制备协议下均属中等（≤ 34%），金属项并不一致地改善 RMSD。两个案例研究界定了当前共价能力的边界：碳酸酐酶 II（3HS4）zn 模式以 2.08 Å 复现磺酰胺–Zn 配位；KRAS G12C（6OIM）的弹头–SG 距离停留在 ~7.2 Å，我们将其作为阴性对照如实报告。LKina 的数值是母函数标度下的经验对接分数（kcal/mol），并非经过标定的结合自由能；多金属位点、桥联配体与大规模独立验证仍是开放性局限，因此我们将 LKina 定位为功能经过验证的工程框架，而非已充分验证的通用预测引擎。
...
上述贡献 1–2 属于软件工程（原子类型覆盖与自足格点管线），3–4 兼有评分函数成分（成对参数替换与后处理 rerank 项）与算法成分（空位方向注入、NAC 约束、两阶段搜索），5–6 属于工作流整合。**范围与适用性。** 配位模型覆盖任意 AD4 金属类型的单核位点；氧化态以显式类型 token（如 `fe2`/`fe3`）处理，或对 Fe/Cu/Mn/Co/V/Mo/Ni 由 BVS 启发式推断；支持的配位几何为四面体、平面四方、直线、八面体与 Jahn–Teller 拉长八面体。共价框架覆盖 Cys/Ser/Lys/Tyr 侧链的亲核加成与取代反应及可逆硼酸酯生成（六种预设，表 4）。LKina 目前不支持：含桥联配体的多金属簇、超出半显式双水模型的水介导配位、金属自旋态/配位场能量差异、共价加合物本身的生成（对接目标为反应前复合物）以及反应后构象弛豫。这些边界连同失败案例在 4.5 节重述。
...
LKina 实现并验证了一个嵌入 AutoDock Vina 1.2.7 的金属感知与共价响应式对接工程框架，且未牺牲其搜索机制与向后兼容性。113 种 AD4 原子类型、四类配位伪原子几何、BVS 氧化态推断、Jahn–Teller 模式、半显式水桥模型与内联 AG4 格点生成器使金属酶对接完全自足。证据支持且仅支持以下结论：全部已实现的金属模式均可解析并完成对接（合成基准 110/110；Vina 1.2.7 仅可解析 34/110），配位误差均值 0.20 Å；在真实 Zn 金属蛋白上，仅 LKina 以 2.19 Å 复现 Zn–N 配位并输出显式配位报告；在 PDB 衍生重对接基准（104 体系，Zn²⁺/Fe³⁺/Cu²⁺）上，启用配位模型后 90.6% 的体系最佳姿态落入金属 3.0 Å 配位球（关闭模型为 27.8%，Vina 1.2.7 为 52.1%），而各引擎 top-1 RMSD 成功率均属中等且金属项并不一致地改善之；P1–P4 共价框架在六种预设上全部跑通并具备真实 NAC 判别；标准对接逐位复现 Vina 1.2.7 能量。证据尚未确立的是规模化情形下的预测准确性：基准仅覆盖单一制备协议下的三种金属类型，尚未与金属专用或共价对接方法进行独立的大规模比较，且所有数值均为经验对接分数而非标定能量。因此，我们将 LKina 定位为一个行为可复现测量的、经过验证的开源软件扩展，而非已充分验证的通用预测引擎；要主张普适准确性或相对优势，需要 4.6 节所列的独立、多种子、协议受控比较。

---

## 6 数据与代码可用性

LKina 开源发布于 https://github.com/LK-Studio1128/LKina ，含源码、构建脚本（macOS arm64/x86_64、Linux x86_64、Windows MSYS2/MinGW）、回归测试套件与发布二进制。许可：合并二进制以 GPL-3.0-or-later 发行（Vina 来源文件保持 Apache-2.0；AG4 引擎与参数数据为 GPL-3.0-or-later；见 `COPYING`/`NOTICE`）。基准数据：PDB 来源的 Zn²⁺/Fe³⁺/Cu²⁺ 重对接集合（§3.6）由 `benchmarks/redock_benchmark/redock_pipeline.py` 从 RCSB PDB [16] 重新生成，JSON 结果存于 `benchmarks/redock_benchmark/results/`。110 模式合成金属基准、功能族实测与四层共价框架基准均可从 `benchmarks/` 复现（见附录 E），所有运行统一使用种子 42 以便精确复现。v1.0.2 快照已存档于 Zenodo：https://doi.org/10.5281/zenodo.22156943 [21]。对应 release tag `v1.0.2-archival`（commit `730376b17641720497bbb42a9829b9f415f89489`）。构建环境：macOS 14 ARM64（Apple clang++ 14，Homebrew libomp）与 Ubuntu 22.04（g++ 11）；依赖版本与 CI 构建矩阵见仓库工作流（`.github/workflows/`）。随基准存档的工件包括输入 PDBQT、预处理脚本、完整命令行、固定种子、原始输出姿态、JSON 结果，以及 3.3 节与 4.5 节讨论的失败案例数据。
