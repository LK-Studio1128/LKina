...
**Results:** We present **LKina**, an open-source C++ extension of AutoDock Vina 1.2.7 that adds metal-coordination docking and reactive covalent docking at the engine level. LKina extends the AD4 atom-type table from ~22 to **113 types** (80+ metals and metalloids), embeds a C++ reimplementation of the AutoGrid 4.2 grid generator (**108** maps per run, removing the external `autogrid4` dependency), introduces four coordination pseudoatom systems (**TZ/SQ/MH/JT**) with heuristic Bond-Valence-Sum (BVS) oxidation-state inference, and implements a four-tier reactive covalent framework (**P1–P4**) with explicit NAC detection, six reaction presets, and a two-phase search option. Evidence is reported at three levels. *Software and numerical verification*: 17/17 regression checks pass, analytical reactive gradients match finite differences to ≤ 4.3 × 10⁻⁸, and standard docking reproduces Vina 1.2.7 mode-1 energies exactly (1HVR −14.54; 3PTB −6.202 kcal/mol). *Synthetic functional coverage*: on a 110-mode metal-coordination benchmark LKina completes **110/110** dockings (Vina 1.2.7 parses only 34/110; the rest fail at atom-type parsing) and recovers the ideal metal–donor distance within 0.5 Å on **108/110** (mean |d−d₀| = 0.20 Å); all six covalent presets complete end-to-end with genuine NAC discrimination. *Real-system comparison*: on 104 PDB-derived metalloprotein redocking cases (Zn²⁺ 22 / Fe³⁺ 47 / Cu²⁺ 35), the coordination model places the best-pose ligand donor within 3.0 Å of the metal in **87/96 (90.6%)** of systems, versus 27/97 (27.8%) for the same engine with metal terms disabled and 50/96 (52.1%) for Vina 1.2.7; top-1 RMSD ≤ 2.0 Å success is moderate for all three engines (≤ 34%) under a lean preparation protocol and is not uniformly improved by the metal terms. Two case studies bracket the current covalent capability: on carbonic anhydrase II (3HS4) zn mode recovers sulfonamide–Zn coordination at 2.08 Å, while on KRAS G12C (6OIM) the warhead–SG distance stalls at ~7.2 Å, which we report as a negative control. LKina’s scores are empirical docking scores on the kcal/mol scale of the parent functions, not calibrated binding free energies; multi-metal sites, bridging ligands, and large-scale independent validation remain open limitations, so we position LKina as an engineering framework with verified functionality rather than a validated general-purpose prediction engine.

**Availability:** https://github.com/LK-Studio1128/LKina (GPL-3.0-or-later; Vina-origin files remain Apache-2.0); prebuilt macOS/Linux/Windows binaries, build scripts, and the full benchmark pipeline are in the repository. The v1.0.2 snapshot is permanently archived on Zenodo (DOI 10.5281/zenodo.22156943).
...
Contributions 1–2 are software engineering (atom-type coverage and a self-contained grid pipeline), 3–4 combine a scoring-function component (pairwise parameter substitution and the post-processing rerank terms) with an algorithmic component (vacant-direction injection, NAC constraints, the two-phase search), and 5–6 are workflow integrations. **Scope and applicability.** The coordination model covers mononuclear sites of any AD4 metal type; oxidation states are handled as explicit type tokens (e.g. `fe2`/`fe3`) or inferred by the BVS heuristic for Fe/Cu/Mn/Co/V/Mo/Ni; supported coordination geometries are tetrahedral, square-planar, linear, octahedral, and Jahn–Teller-elongated octahedral. The reactive framework covers nucleophilic addition and substitution at Cys/Ser/Lys/Tyr side chains and reversible boronate formation (six presets, Table 4). LKina does not currently handle: multi-metal clusters with bridging ligands, water-mediated coordination beyond the semi-explicit two-water model, metal spin-state or ligand-field energetic differences, covalent adduct formation itself (docking targets the pre-reaction complex), or post-reaction conformational relaxation. These boundaries are restated with failure cases in Section 4.5.
...
with default weights $W_{\text{vdW}}=0.1662$, $W_{\text{Hbond}}=0.1209$, $W_{\text{elec}}=0.1406$, $W_{\text{desolv}}=0.1322$, and $W_{\text{tor}}=0.2983$ (AutoDock4 [3]). In grid-based docking, the receptor contribution is pre-computed on a Cartesian grid and the ligand energy during the search is obtained by trilinear interpolation, which keeps the per-pose evaluation effectively O(N_ligand). LKina re-implements this entire grid pipeline in-engine (Section 2.3). **A note on units and terminology.** Throughout this paper, "score", "affinity", and "energy" refer to the empirical docking score of the parent function, reported in kcal/mol by convention of the AutoDock family. These values are not calibrated binding or reaction free energies, and scores from different functions (vina, vinardo, AD4, and the metal/covalent-extended AD4) are not directly comparable to one another; cross-function comparisons in this work are therefore made on geometry (coordination distances, RMSD) rather than on score equality.
...
**Composition with the base scoring functions.** The metal terms enter the AD4 score by *pairwise parameter substitution*, not as an additive layer: for each metal–donor probe pair, the standard vdW/solvation table entry is replaced by the `nbp_r_eps` override when the affinity maps are built, so no interaction is counted twice and the electrostatic and hydrogen-bond terms are untouched. Under `--scoring vina` or `vinardo`, no metal terms exist at all (Section 3.7 quantifies the consequence: a 12.5 Å coordination miss on 4JC). The post-processing rerank terms of Section 2.7 add a separate correction to the final ranking only. Protonation states, tautomer choices, and crystallographic water retention are inherited from the input PDBQT preparation and remain the user's responsibility; the benchmark in Section 3.6 states its preparation protocol explicitly.
...
**Nature and limits of the BVS step.** The BVS calculation here is a *geometric heuristic for mode selection*, not a charge-assignment or quantum-chemical determination: it reads only donor distances from the input PDBQT and is therefore sensitive to missing or extra donors (unsaturated or over-coordinated sites), protonation ambiguity of His/Asp/Glu donors, and incorrect metal symbols in the input structure. The deviation threshold (1.25 valence units) is intentionally loose; when no candidate state falls below it, the engine falls back to donor counting (step 3 above), and the user can override inference entirely with an explicit `--metal_mode` token. The 14/14 result of Section 3.2.2 therefore characterizes the heuristic on designed, chemically clean inputs and should not be extrapolated to noisy experimental structures.
...
**Task definition and terminology.** Throughout this work, *covalent docking* means docking the **pre-reaction complex**: the ligand carries an unreacted warhead (electrophilic center) and the receptor an unmodified nucleophilic atom (e.g. Cys SG), and no bond is ever formed during the search — output poses remain non-bonded geometries, and the covalent adduct itself is represented only by convergence to the target bond length $r_0$. We use four terms strictly: (i) the *pre-reaction complex* is the input state above; (ii) a *near-attack conformation (NAC)* is a pose whose nucleophile–electrophile geometry satisfies the reaction criteria (distance < 3.0 Å and attack angle within ±25° of the preset target; Table 4) [10,11]; (iii) a *reactive pose* is an output pose flagged by the NAC detector as satisfying those criteria — the flag is geometric and carries no claim about chemical reactivity; and (iv) the *covalent adduct* is the post-reaction bonded species, which LKina does not model. Attack angles follow the Bürgi–Dunitz trajectory for addition (≈109.5°) and the backside trajectory for SN2 substitution (180°).
...
which attracts the pair toward the target bond length $r_0$ and applies a quadratic penalty when the ligand drifts beyond $r_0+\sigma$. The gradient with respect to the ligand coordinates is computed analytically. The amplitude $A$ (the "reactive strength" or attractor depth) carries the kcal/mol scale of the parent AD4 score; it is a dimensioned *guidance weight of the empirical score*, not a physically derived interaction energy, and it is linear by construction (the term is additive in $A$; Section 3.7 verifies the linear score response). The defaults (Table 4; e.g. a 10 kcal/mol Gaussian well for `cys_michael`) keep the anchor contribution comparable to a strong hydrogen-bond/vdW well; at substantially larger strengths the anchor can dominate the total score and distort pose ranking, which we quantify in Section 3.7 and caution against.
...
with the target angle $\theta_0$ adapted to the mechanism: 180° for SN2 backside attack, ≈109.5° for Michael addition (the Bürgi–Dunitz trajectory toward the tetrahedral transition state). Receptor-side and ligand-side frame atoms (`--reactive_frame_atom`, `--reactive_lig_frame_atom`) define the reference frames; the gradient of $\cos\theta$ w.r.t. the ligand atom is
...
LKina extends the Vina 1.2.7 C++14 code base as a set of new modules plus targeted extensions to existing ones; the description below is organized by responsibility rather than by diff size. In the docking pipeline, the modules slot in as follows: **(1)** receptor/ligand parsing (`main.cpp`) detects metal types and, when `--metal_mode` is absent, runs BVS oxidation-state inference (Section 2.5); **(2)** the AG4 engine (`ag4_engine.{h,cpp}`) enumerates active metal modes, injects coordination pseudoatoms and metal-pair `nbp_r_eps` overrides, and computes affinity grids (`embedded_ad4_grid.cpp`), replacing the external `autogrid4` call; **(3)** the grid cache (`ad4cache.{h,cpp}`) holds the metal state and the post-processing rerank terms (Section 2.7); **(4)** the reactive framework types (`reactive_types.h`) and the CLI reactive flags add the P1–P4 constraint layer and NAC detection on top of the grid score (Sections 2.9–2.10); and **(5)** the search itself (Vina's Monte Carlo + L-BFGS, OpenMP over poses) is inherited unchanged. `atom_constants.h` and `ad4_parameter_data.cpp` hold the 113-type table and the embedded force-field text parsed at run time. Pseudoatom injection happens during single-threaded receptor parsing, so no new thread-safety concerns arise.

**Compatibility.** LKina reads and writes standard Vina/AD4 PDBQT structures and AutoGrid 4.2-format `.map` files (its own grids are byte-compatible for external reuse); Meeko- or Open-Babel-prepared ligand PDBQTs are accepted unchanged. All standard Vina 1.2.7 CLI flags are preserved (Section 2.12), and `--scoring vina`/`vinardo` remain available for backward-compatibility verification. There are no new Python bindings; scripted use goes through the CLI (as in all benchmark scripts of this work). Behavior for metal-free, non-reactive systems is numerically identical to Vina 1.2.7 (Section 3.5), so drop-in substitution within existing Vina workflows, including LKDock-style GUI integration, is supported; the one known packaging caveat is described below.
...
Results are organized as an evidence hierarchy rather than as a feature list. Section 3.1 establishes *software correctness* (regression tests, build matrix); Section 3.2 adds *numerical verification* of the metal/gradient machinery (the v1.0.2 engine fixes and the analytical-gradient audit). Sections 3.2–3.4 report *functional validation on synthetic and preset systems*, which can demonstrate that the machinery works as designed but not real-world predictive accuracy. Sections 3.3, 3.6, and 3.7 provide the *scientific evidence on real systems* — a three-engine comparison, a 104-case PDB redocking benchmark, and parameter-stability measurements — whose scale remains the main limitation of this work (Section 4.5). We keep the software-verification material visible rather than relegated to supplementary material because the v1.0.2 fixes (Section 3.2.1) change numeric results reported elsewhere in the paper.

### 3.1 Software verification: regression tests and build matrix

This section establishes only that the implementation is correct as written; it does not by itself support any claim of predictive validity (see the evidence hierarchy above). LKina's validation layer (17 automated regression checks in `tests/`, per engine design document §5.1) is organized into six groups: A, 4 global docking-energy regressions at fixed seed/exhaustiveness (2 of which reproduce the Vina 1.2.7 values exactly); B, 5 score-decomposition checks on crystal conformations (`--score_only`); C, 3 `lig_atom` gradient vs finite-difference validations; D, 1 `target_angle` parameter-effect test; E, 1 `hybrid_vdw_scale` continuous-scaling test; and F, 3 `lig_frame_atom` gradient validations. All 17 checks pass on the release binary (macOS ARM64, seed 42; `tests/reactive_regression.sh`). In addition, this work independently re-executed the highest-value checks against the live binary: the reactive gradient check via `--reactive_gradcheck` (maximum |analytic − numeric| deviation **2.4 × 10⁻¹⁶–4.3 × 10⁻⁸** across the seven force components of all six presets; Section 3.2.2), two global docking-energy regressions on real organic systems (Section 3.5), and score-decomposition verification during the energy-scale audit (`INTER + torsions = VINA RESULT` holds exactly). The engine compiles with zero errors and zero warnings (`-Wall -Wno-long-long -O3 -fopenmp`) on macOS 14 (Apple M2, clang++ 14), macOS 13 (Intel), and Ubuntu 22.04 (g++ 11).
...
Second, we report the RMSD results candidly, including the unfavorable ones: under this lean preparation pipeline (Open Babel without protonation-state optimization, rigid receptor, exhaustiveness 8), all three engines achieve only moderate global pose-redocking success (Fe³⁺/Cu²⁺ ≤ 34%, Zn²⁺ ≤ 32%), and LKina metal mode does not improve global RMSD success over its own AD4 baseline on every subset. The absolute rates fall below those of literature redocking studies that use refined protonation workflows (AutoDockTools/Meeko with manual tuning) and higher exhaustiveness; a systematic comparison of preparation protocols is beyond the scope of this work. We therefore present the RMSD column as a like-for-like engine comparison under a single fixed protocol, and the donor–metal distance as the metric specific to the coordination-model claim. **On stochasticity:** the full cohort was run once at seed 42 for computational economy; seed sensitivity was quantified on representative systems (five seeds at exhaustiveness 8 give an affinity range of 0.010 kcal/mol on 3HS4 zn and 0.053 kcal/mol on 6OIM vina, Section 3.7), which bounds the stochastic contribution to score reproducibility but not to pose identity. Extending the multi-seed protocol (≥ 5 seeds with mean ± s.d. and paired statistics per system) to the entire cohort, together with top-1/3/5 success rates and per-mode analysis, is scheduled as the immediate next step and listed in Section 4.6.
...
Three families of stability and parameter-response measurements were run against the fixed binary (v1.0.2 engine; all scripts, JSON results, and raw poses are deposited in `benchmarks/`, with summary figures S5–S10).
...
**Search-protocol stability (Figures S6–S7).** Across exhaustiveness 1–32 on 3HS4 (zn mode, 3 seeds each), the best-affinity standard deviation falls to **σ ≈ 0.003 kcal/mol at exhaustiveness 8** (and remains ≤ 0.010 even at exhaustiveness 1), while runtime grows near-linearly — the default exhaustiveness 8 is well past the convergence knee. Five-seed variance at exhaustiveness 8 is **0.010 kcal/mol** (3HS4, zn) and **0.053 kcal/mol** (6OIM, vina), both orders of magnitude below the ~1 kcal/mol accuracy of any current scoring function; single-seed results are therefore reproducible at the scoring-accuracy scale.

**Parameter response (Figures S8–S9).** Sweeping `--metal_soft_weight` over 0–0.5 on 3HS4 shifts the best-pose Zn distance by only 2.08 → 2.07 Å and the affinity by −13.319 → −13.316 kcal/mol (within noise): the search-time soft constraint is non-perturbative once coordination is already correct, consistent with its intended role of guiding initially displaced poses (Section 4.4). Sweeping the `--metal_bias` attractor strength 0 → 8 kcal/mol deepens the affinity monotonically (−13.32 → −14.85) while the Zn distance stays at 2.08–2.11 Å, i.e. the attractor works as designed without corrupting geometry. The covalent anchor strength responds linearly (best score −0.63 → −8.68 kcal/mol for attractor strength 2 → 16 on the cys_michael system), so anchoring strength is quantitatively tunable. The upper end of this sweep also illustrates the over-attraction risk noted in Section 2.9: at strength 16 the anchor term alone accounts for most of the best score, and pose identity (not only the score) would need re-examination before such settings are used; the preset defaults (Table 4) avoid this regime.

**Hotspot-target cases (Figure S10).** (i) *Carbonic anhydrase II/AZM (3HS4)*: zn mode recovers the sulfonamide–Zn coordination at 2.08 Å (crystal Zn–N 1.94 Å) with affinity −13.32 kcal/mol versus the experimental ≈ −10.8; adding the metal bias deepens the well (−13.69) without breaking the coordination distance (2.10 Å) — the zn + metal-bias workflow fits zinc-enzyme screening directly. (ii) *KRAS G12C covalent (6OIM, cys_michael preset)*: we report this case as a **negative control** for the reactive workflow. Construction: in the deposited structure (PDB 6OIM) the acrylamide inhibitor is covalently bonded to Cys 12 SG; the receptor was prepared rigid with the co-crystallized ligand removed, so the system does not contain a genuine pre-reaction complex (the nucleophile position and pocket geometry reflect the post-reaction state), and recovering a reaction-competent pose is not expected under this preparation. Measurement: the reported ~7.2 Å is the minimum Euclidean distance in the top-ranked docked pose between the designated reactive atom pair — the warhead electrophilic β-carbon C17 (`reactive_lig_atom = name:C17`) and the SG sulfur of Cys 12 (`reactive_rec_atom = A:12:SG`) — computed from the docked model-0 coordinates in `run_hotspot_cases.py` (`nearest_atom_dist`). Physical meaning: the distance is a search-progress surrogate for the target bond length ($r_0$ = 1.82 Å); at ~7.2 Å no output pose satisfies the NAC criteria (< 3.0 Å, ±25°), so no pose qualifies as a reactive pose and none is labeled as one. The reactive modes all complete (best −19.1 to −19.3 kcal/mol, indistinguishable from plain AD4 on this metal-free target), confirming that the constraint machinery does not crash or fabricate NACs on a case where the geometry is unattainable. The likely causes of the failed guidance are the rigid-receptor preparation and the default attractor well being too weak inside the ~20 Å box with 4 rotatable warhead bonds; a re-scan with a wider/stronger attractor or the hybrid mode with an explicit bond-length term is listed in Section 4.6.
...
- **Figure S10.** Hotspot-target cases. (A) KRAS G12C (6OIM, cys_michael): all five docking modes complete, but the warhead C17–SG12 distance converges only to ~7.2 Å (dashed line = covalent bond length ~1.8 Å); a negative control reported and dissected in Section 3.7. (B) Carbonic anhydrase II (3HS4): zn mode places the AZM sulfonamide N at 2.08 Å of the catalytic Zn²⁺ (crystal 1.94 Å); the metal bias deepens the score without disturbing the coordination distance.
...
LKina differs from AutoDock4Zn [6] in *coverage* (any metal rather than Zn²⁺ alone; four pseudoatom geometries; Jahn–Teller and water-bridge terms) and from MetalDock [7] in *architecture* (engine-level atom types and grids versus an external pre-processing protocol; no external `autogrid4`). The 110-mode synthetic benchmark quantifies the coverage gap directly: 76 metal-coordination systems fail even Vina 1.2.7's parsing, whereas LKina docks all of them with a mean coordination error of only 0.20 Å. Unlike CovDock, LKina is open source, requires no pre-built covalent adducts, and reports NAC status per pose. Other representative docking programs, the CHARMm grid-based CDOCKER [19] and the genetic-algorithm GOLD [20], provide reference accuracy baselines for covalent and flexible docking, but their commercial licenses or external pre-processing dependencies preclude a direct engine-level comparison; this work therefore runs LKina and Vina 1.2.7 head-to-head under identical conditions (same receptor/ligand files, fixed seed, top-1 RMSD; Sections 3.3–3.6) and treats the CDOCKER/GOLD literature accuracy as a qualitative reference rather than a re-run. To our knowledge, LKina is currently the only open-source engine that integrates, at the engine level, metal-coordination docking, NAC-constrained reactive docking, and a fully self-contained grid pipeline. **Scope of the comparison.** No metal-specialized or covalent external method (AutoDock4Zn protocol, MetalDock, CovDock, GOLD) was re-run in this work, and the baselines are deliberately simple: the same engine with its extensions disabled (`--no_auto_metal`) isolates the coordination-model contribution, and stock Vina 1.2.7 provides the open-source standard. We did not implement post-hoc pose rescoring or distance-filter baselines; adding such simple baselines, plus at least one metal-specialized method on a shared Zn subset, is part of the planned benchmark expansion (Section 4.6), under a uniform protocol (identical preparation, box, seed, exhaustiveness, pose count, and success criterion).

### 4.2 The synthetic benchmark: what it does and does not measure

The synthetic metal-coordination systems are deliberately minimal (a metal, a partial donor shell, and a probe ligand) and therefore **measure coverage and correct operation, not predictive accuracy**: they verify that every metal mode parses, that pseudoatom injection and nbp overrides load without error, and that the search completes with finite scores and emits coordination remarks. The coordination-distance residuals (mean 0.20 Å across 110 modes) are lower-bound estimates because the systems lack the protein pocket that makes the metal site geometrically unique in real complexes. Real-system accuracy is established separately by the 4JC comparison (Section 3.3) and the PDB-derived Zn/Fe/Cu redocking datasets (Section 3.6).
...
(i) **Multi-metal coupling.** Modes are applied per site independently; bridging ligands of dinuclear centers (μ-aqua, μ-oxo) may be double-counted in rerank scores, and the BVS inference remains single-center (Mo–Fe nitrogenase and the tetranuclear Mn cluster of photosystem II are not yet handled rigorously; Ni–Fe hydrogenase receives a single-center Ni approximation). (ii) **Metal-as-ligand scope.** The reverse-potential extension is a conservative, grid-based approximation: MetalDock-style ligand-side dummy atoms and internal geometric constraints are not ported, and dedicated benchmarks for Pt/Ru/Tc/Re/Gd agents are pending. (iii) **Reactive score is not thermodynamic**: reactive penalty terms must not be interpreted as binding-affinity or activation-energy predictions (Section 2.9); large attractor strengths can dominate the score and distort ranking (Section 3.7). (iv) **Synthetic benchmark granularity**: the 110-mode systems are minimal by design; a full PDB-derived multi-metal benchmark would require additional infrastructure. (v) **Statistical robustness**: the redocking cohort was run once per engine (seed 42, exhaustiveness 8/16 depending on section); measured seed sensitivity bounds the score-level stochastic contribution (Section 3.6) but not pose-level variability, and no confidence intervals or paired tests are reported yet.

**Published failure cases.** Failures are listed here with the same prominence as the successes. (a) *Global pose recovery*: on the 104-system redocking benchmark, top-1 RMSD ≤ 2.0 Å success is 0% (Zn²⁺), 12.8% (Fe³⁺), and 11.4% (Cu²⁺) for LKina metal mode — the coordination model recovers metal-adjacent geometry (90.6% within 3.0 Å) but does not by itself solve global pose prediction under this lean protocol. (b) *Covalent guidance*: on the 6OIM negative control (Section 3.7), reactive docking completes but leaves the warhead–nucleophile distance at ~7.2 Å; no NAC-competent pose is produced. (c) *Synthetic edge cases*: 6 of 110 synthetic modes (four LIN, one TZ, one MH) show no measurable coordination well at the probed donor position (Figure S1); these are linear or edge-geometry sites whose tabulated overrides are weakest. (d) *Metal-as-ligand RMSD*: on the 20-complex Pt/Pd/Ru/Os/Re pool, global RMSD ≤ 2.0 Å succeeds in only 1/20 (flexible metal complexes defeat a global-RMSD metric; the coordination-competence metric is the meaningful one for this class, Figure S4). (e) *Preparation sensitivity*: a receptor prepared with 1,349 UNL/UNK co-ligand atoms corrupted absolute energies by ~+990 kcal/mol before the clean-receptor rerun (Section 3.3) — input preparation quality gates every result above.
...
LKina implements and validates an engineering framework for metal-aware and covalent-reactive docking inside AutoDock Vina 1.2.7, without sacrificing its search machinery or backward compatibility. A 113-type AD4 atom system, four coordination pseudoatom geometries, BVS oxidation-state inference, Jahn–Teller modes, a semi-explicit water-bridge model, and an inline AG4 grid generator make metalloenzyme docking self-contained. The evidence supports the following, and only the following, conclusions: every implemented metal mode parses and completes (110/110 synthetic modes; Vina 1.2.7 parses 34/110) with a mean coordination error of 0.20 Å on those systems; on a real Zn metalloprotein, LKina alone recovers Zn–N coordination at 2.19 Å with an explicit coordination report; on a PDB-derived redocking benchmark (104 systems, Zn²⁺/Fe³⁺/Cu²⁺), enabling the coordination model places the best-pose ligand donor within the 3.0 Å coordination sphere in 90.6% of systems, versus 27.8% for the same engine with the model disabled and 52.1% for Vina 1.2.7, while top-1 RMSD success remains moderate for all engines and is not uniformly improved; the P1–P4 reactive framework completes all six presets with genuine NAC discrimination; and standard docking reproduces Vina 1.2.7 energies exactly. What the evidence does not yet establish is predictive accuracy at scale: the benchmarks cover three metal types under one preparation protocol, no independent large-scale comparison against metal-specialized or covalent methods has been run, and all scores are empirical docking scores rather than calibrated energies. We therefore position LKina as a verified, open-source software extension whose coordination-geometry behavior is reproducibly measurable, not as a validated general-purpose prediction engine; claiming broad accuracy or superiority will require the independent, multi-seed, protocol-controlled comparisons outlined in Section 4.6.

---

## 6. Data and code availability

LKina is open source at https://github.com/LK-Studio1128/LKina, including the source code, build scripts (macOS arm64/x86_64, Linux x86_64, Windows MSYS2/MinGW), the regression suite, and release binaries. License: the combined binary is distributed under GPL-3.0-or-later (Vina-origin files remain Apache-2.0; the AG4 engine and parameter data are GPL-3.0-or-later; see `COPYING`/`NOTICE`). Benchmark data: the PDB-derived Zn²⁺/Fe³⁺/Cu²⁺ redocking sets (Section 3.6) are regenerated by `benchmarks/redock_benchmark/redock_pipeline.py` from the RCSB PDB [16], with JSON results deposited in `benchmarks/redock_benchmark/results/`. The 110-mode synthetic metal benchmark, the feature-family measurements, and the four-tier covalent framework benchmark are all reproducible from `benchmarks/` (see Appendix E); every run used seed 42 for exact reproducibility. The v1.0.2 snapshot is archived on Zenodo: https://doi.org/10.5281/zenodo.22156943 [21], corresponding to release tag `v1.0.2-archival` (commit `730376b17641720497bbb42a9829b9f415f89489`). Build environment: macOS 14 ARM64 (Apple clang++ 14, libomp via Homebrew) and Ubuntu 22.04 (g++ 11); dependency versions and CI build matrices are pinned in the repository workflows (`.github/workflows/`). Deposited per-benchmark artifacts include input PDBQTs, preprocessing scripts, complete command lines, fixed seeds, raw output poses, JSON results, and the failure cases discussed in Sections 3.3 and 4.5.
...
| Hotspot 6OIM | KRAS G12C covalent | warhead C17–SG12 distance (negative control, Section 3.7) | ~7.2 Å | `run_hotspot_cases.py` |
